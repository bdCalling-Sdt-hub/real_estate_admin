...
const CreateServices = () => {
  ...
  const [formData, setFormData] = useState({ serviceIds: [] });

  const tabs = ["Services", "Address", "Contact Info", "Confirm"];
  const tabContent = [
    <CreateANewOrder key={0} formData={formData} setFormData={setFormData} setActiveTab={setActiveTab} />,
  ...
  return (
    <div className="bg-white p-4">
      <div className="flex justify-center" style={{ display: activeTab === 0 ? "none" : "flex" }}>
      ...
      <TabFooter activeTab={activeTab} handleNext={handleNext} handlePrevious={handlePrevious} tabs={tabs} />
    </div>
  );
};
...
const TabHeader = ({ tabs, activeTab }) => (<div style={{ display: "flex", marginBottom: "20px" }}>
  {tabs.map((tab, index) => (
    <div className="flex" key={index}>
      <div
        style={{
          padding: "6px 20px",
          border: (activeTab - 1) === index ? "1px solid #9B3C7B" : "1px dashed #9B3C7B",
...
const TabFooter = ({ activeTab, handleNext, handlePrevious, tabs }) => (<div className="flex justify-center mt-11">
  <div className="flex gap-5" style={{ display: activeTab === 0 ? "none" : "flex" }}>
    <button
